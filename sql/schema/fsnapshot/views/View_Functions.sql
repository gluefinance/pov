CREATE OR REPLACE VIEW fsnapshot.View_Functions AS
SELECT
...
-- Do not include pg_catalog, information_schema, or our own schema "fsnapshot"
WHERE pg_catalog.pg_namespace.nspname NOT IN ('pg_catalog','information_schema','fsnapshot')
-- Skip aggregates as they cannot be defined using pg_get_functiondef
AND pg_catalog.pg_proc.proisagg IS FALSE
-- Do not include the fsnapshot API-functions in the selection
AND (pg_catalog.pg_namespace.nspname <> 'public' OR pg_catalog.pg_proc.proname <> 'fsnapshot')
;
